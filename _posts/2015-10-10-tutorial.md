...
Quick rundown of vocabulary `meta` is the tag (like `html`, `head`, `body`), description is an attribute (which we'll see more instances of later). When they're shown in a browser, these tags are what we call **elements**.
